...
| Keep explanation proportionate, let it fade, and keep it beside its step | Reviewed the course format against published guidance before starting Module 5. The format itself matched the evidence well: Parts give learner-paced segments, headings and the two callout icons signal structure, nearly every lesson ends a step with a visible "Confirm that" result, prerequisites are stated, recovery help sits in "If something differs", and exercises have the learner modify and explain. What had drifted was explanation density, and only in Module 4. Measured across all 37 lessons, Modules 0 to 3 average 3.4 callouts with a maximum of 8 and about 0.44 words of explanation per word of instruction; Module 4 averages 14.6 callouts with a maximum of 29 in Lesson 4.6 and 1.71 words of explanation per word of instruction, about four times as dense. That runs against the coherence principle, supported in 23 of 23 experiments with a median effect size of 0.86 in Mayer and Fiorella's chapter of the Cambridge Handbook of Multimedia Learning (2014), and against the expertise reversal effect described by Kalyuga, under which guidance that helps novices becomes extra load as they gain knowledge, so explanation should fade through a course rather than grow. Seven Module 4 callouts begin "Step N is...", placing explanation away from the step it concerns, against spatial contiguity, supported in 22 of 22 experiments with a median effect size of 1.10 in the same chapter. The cause was identified as authoring process rather than format: measurements taken to validate Module 4 were written into the lessons as callouts, alongside design rationale and re-explanations of earlier lessons, when the measurements belong here as evidence. Separately, only 11 of 37 lessons ask the learner to predict anything, and every one of those predictions sits in the learner exercise rather than before the first run of new code. PRIMM, a widely used approach for structuring programming lessons, orders work as predict, run, investigate, modify, make, to counter novices writing programs before they can read them; its outcome data could not be retrieved during this review and is not relied on here. Diataxis, a documentation framework rather than learning research, was consulted for its insistence that every step produce a visible result and that a tutorial work every time, both of which the course already does; its stronger rule that a tutorial should carry almost no explanation was not adopted, because the course is written for learners with no prior experience and the expertise reversal research supports more guidance for them, not less. A follow-up pass found that two of the rules first added, keeping explanation beside its step and letting it fade, already existed in the Build section of `course_workflow.md`, so Module 4 broke standing rules rather than missing ones; the duplicates were removed and the existing rules were strengthened with the concrete symptom of a callout beginning "Step 8 is..." and with the note that the rule matters more as the course goes on. The same pass added what was genuinely missing. In `course_workflow.md`: check before drafting whether an earlier lesson already explains, tests or audits the same thing; give the learner exercise a shape, predict then try then explain, experimenting on a copy or a second instance where possible and ending with the exact value to restore when it cannot; scope the verification checklist to the end state and the understanding built rather than restating each build step; show one way to do each thing, since offering alternatives is a detour; end each lesson part with something the learner can see or run; and hand repeated patterns to the learner once a technique has been shown in full, which the expertise reversal research supports by finding that detailed worked-out steps help novices and hinder more advanced learners. In `AGENTS.md`, which governs the assistant: keep validation evidence out of lesson text, and validate under the learner's conditions rather than this repository's. The rules are framed as signals to re-read a lesson rather than as limits. No validated lesson was rewritten: Module 4 is the natural first target of the planned simplification run. |
| Apply the proportionate-explanation rules to the validated lessons | The rules added after the format review were applied to fifteen validated lessons. Module 4 was cut hardest: callouts fell from 12, 14, 9, 10, 15, 29 and 25 to 9, 8, 8, 9, 7, 13 and 12 across Lessons 4.1 to 4.7, taking the module's mean from 14.6 to 8.6 and its explanation-to-instruction ratio from 1.71 to 0.81, and all seven callouts beginning "Step N" were moved beside their steps. What went was design rationale, measured evidence, alternatives, and re-explanations of earlier lessons, all of it already recorded in this roadmap; what stayed was each first introduction of a concept and each warning about a mistake the learner is about to make, shortened and placed beside the step it concerns. Four checklist items were removed from Lesson 4.6, three from 4.7 and two merged into one in 4.2, each one restating a build step or duplicating another item. A one-line prediction was folded into the first run of new work in Lessons 2.5, 2.8, 2.9, 2.11, 3.2, 3.3, 3.4, 3.5 and 4.1 to 4.7, written into the existing run step so that no step was added or renumbered. Every edited lesson was checked mechanically against its previous version: identical step numbering, headings and code fences, byte-identical code inside the fences, no callout beginning "Step N", no doubled blank lines outside code, and no new line over 79 characters. Because no step, value or code listing changed, the lessons keep their Validated status. Two suggestions were not applied after inspection. The Module 3 checklists are long because they check the shape of a lot of new code, which the Verify rules require, and a scan found none that restate a step. And the Lessons 4.3, 4.4 and 4.7 exercises were left as they are: swapping or offsetting the level is the point of those exercises, so there is no second instance to experiment on instead. Lessons 0.2, 0.4, 2.2 and 2.3 were re-read and left unchanged, since every one of their callouts introduces a term for the first time. |
| Indent lesson code with tabs so it can be pasted into a learner's file | A final audit found that every GDScript block in all 37 lessons was indented with spaces while all four project scripts use tabs, a split that was total on both sides and had gone unnoticed because each half is internally consistent. It was measured against the running engine rather than assumed: an all-spaces file loads fine on its own, but appending a space-indented block to a tab-indented file gives `Parse Error: Used space character for indentation instead of tab as used before in the file.` That is the normal case in this course, because lessons add code to a file the learner already typed into, and Godot's editor indents typed code with tabs. The failure was reproduced by taking `level.gd` as Lesson 4.7 finds it and pasting Lesson 4.7's own block into it, which fails with that error, then repeating with the corrected block, which does not. All 234 indented lines across 98 blocks were converted, leaving the spaces that place a fence inside a numbered step untouched and changing only the indentation within the code. The conversion was verified to be whitespace-only: identical content with all whitespace stripped, byte-identical lines outside code blocks, unchanged line counts, and an empty `git diff --ignore-all-space`. Lesson 4.6's listing is now byte-identical to `levels/moving_platform.gd`, and each of Lesson 4.7's four blocks appears verbatim in the script it belongs to. `course_workflow.md` carries the convention. Two findings from the same audit were left alone: 56 lines exceeding the 79-column convention, all in Modules 0 to 2 and cosmetic, and three lessons saying "enable Visible Collision Shapes" where it is already on, against four that say "confirm it is enabled". |
